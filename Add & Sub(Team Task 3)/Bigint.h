...
bigintadd(string str1)
{
    for(int i = 0; i < str1.size(); ++i){
        if(str1[i] == '0' && (number1 == "" || number1 == "-") && i != str1.size()-1){
            continue;
        }else{
            number1 += str1[i];
        }
 ...
 bigintadd operator+(bigintadd mat)
 {
    ...
    bigintadd n(sum(str_1 , str_2));
    return n;
}
//done

friend ostream& operator<<(ostream& out, const bigintadd& n);

bigintadd operator-(bigintadd test)
        {
            string str_1 = number1;
            string str_2 = test.getNumber();
            bigintadd n(Diff2(str_1 , str_2));
            return n;
        }
...
ostream& operator<<(ostream& out, const bigintadd& n){
    out << n.getNumber();
    return out;
}
#endif
